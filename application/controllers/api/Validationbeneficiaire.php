...
class Validationbeneficiaire extends CI_Controller {
	...
	public function upload_validationdonneesbeneficiaire() {	
		...
		if (isset($_FILES['file']['tmp_name'])) {
			...
			$valeur_retour["erreur_titre"] = $retour["erreur_titre"];
	...
	public function controler_donnees_beneficiaire($filename,$directory,$adresse_mail) {	
		...
		$erreur_doublon_menage=0;
		$erreur_doublon_individu=0;		
		$erreur_titre=0;
		foreach($rowIterator as $row) {
			...
			if($ligne==4) {
				 ...
				 $rowIndex = $row->getRowIndex ();
					...
					if(strtolower($numero_ordre)!='ordre') {
						$erreur_titre=$erreur_titre + 1;
					}
					if(strtolower($identifiant_appariement)!='identifiant') {
						$erreur_titre=$erreur_titre + 1;
					}
					if(strtolower($nom)!='nom') {
						$erreur_titre=$erreur_titre + 1;
					}
					if(strtolower($prenom)!='prénom' && strtolower($prenom)!='prenom') {
						$erreur_titre=$erreur_titre + 1;
					}
					if(strtolower($chef_menage)!='chef ménage (o/n)') {
						$erreur_titre=$erreur_titre + 1;
					}
					if(strtolower($date_naissance)!='date naissance') {
						$erreur_titre=$erreur_titre + 1;
					}
					if(strtolower($age)!='age (si date naissance inconnue)') {
						$erreur_titre=$erreur_titre + 1;
					}
					if(strtolower($sexe)!='sexe (h/f)') {
						$erreur_titre=$erreur_titre + 1;
					}
					if(strtolower($sexe)!='Situation matrimoniale (célibataire / marié / veuf(ve)/ divorcé)') {
						$erreur_titre=$erreur_titre + 1;
					}
					if(strtolower($cin)!='c.i.n') {
						$erreur_titre=$erreur_titre + 1;
					}
					if(strtolower($profession)!='profession') {
						$erreur_titre=$erreur_titre + 1;
					}
					if(strtolower($adresse)!='adresse') {
						$erreur_titre=$erreur_titre + 1;
					}
					if(strtolower($surnom)!='surnom chef ménage') {
						$erreur_titre=$erreur_titre + 1;
					}
					if(strtolower($niveau_classe)!='Niveau de classe ( 01 : cp1 …. 12 : terminale .. 13 : université)') {
						$erreur_titre=$erreur_titre + 1;
					}
					if(strtolower($langue)!='langue(autre que malagasy)') {
						$erreur_titre=$erreur_titre + 1;
					}
					if(strtolower($revenu)!='revenu mensuel') {
						$erreur_titre=$erreur_titre + 1;
					}
					if(strtolower($depense)!='dépense mensuel') {
						$erreur_titre=$erreur_titre + 1;
					}
					if(strtolower($date_inscription_detail_chaine)!='date inscription (en tant que bénéficiaire)') {
						$erreur_titre=$erreur_titre + 1;
					}
					if(strtolower($telephone)!='téléphone') {
						$erreur_titre=$erreur_titre + 1;
					}
					if(strtolower($handicap_visuel)!='handicap visuel (malvoyant / aveugle)') {
						$erreur_titre=$erreur_titre + 1;
					}
					if(strtolower($handicap_auditif)!='handicap auditif (malentendant / sourd)') {
						$erreur_titre=$erreur_titre + 1;
					}
					if(strtolower($handicap_parole)!='handicap parole (bègue / muet)') {
						$erreur_titre=$erreur_titre + 1;
					}
					if(strtolower($handicap_moteur)!='handicap moteur (paralytique / infirme)') {
						$erreur_titre=$erreur_titre + 1;
					}
					if(strtolower($handicap_mental)!='handicap mental (insensé / aliéné / fou)') {
						$erreur_titre=$erreur_titre + 1;
					}
					if(strtolower($nom_enqueteur)!='nom enqueteur') {
						$erreur_titre=$erreur_titre + 1;
					}
					if(strtolower($date_enquete)!='date enquete') {
						$erreur_titre=$erreur_titre + 1;
					}
					if(strtolower($indice_vulnerabilite)!='vulnérabilité (faible;moyen;elevé;très élevé )') {
						$erreur_titre=$erreur_titre + 1;
					}
			}
			if($ligne >=2) {
				 ...
				 $cellIterator->setIterateOnlyExistingCells(false);
				 $rowIndex = $row->getRowIndex ();
				 // lecture acteur , intervention, date d'envoi, menge/groupe/individu
				if($ligne ==2) {
					// Contrôle partenaire / intitulé intervention / Date 
					 $cellIterator = $row->getCellIterator();
					 $cellIterator->setIterateOnlyExistingCells(false);
				...
				// Lecture découpage administratif
				if($ligne==3) {
					 $cellIterator = $row->getCellIterator();
					 $cellIterator->setIterateOnlyExistingCells(false);
					 $rowIndex = $row->getRowIndex ();
				...
				// Lecture détails 
				if($ligne>=5) {
					if($erreur_titre==0){   // c'est à dire le format est OK
						 $cellIterator = $row->getCellIterator();
						 $cellIterator->setIterateOnlyExistingCells(false);
						 $rowIndex = $row->getRowIndex ();
						foreach ($cellIterator as $cell) {
							if('B' == $cell->getColumn()) {
								...
								if(isset($date_inscription_detail_beneficiaire) && $date_inscription_detail_beneficiaire>"") {
									if(PHPExcel_Shared_Date::isDateTime($cell)) {
										 $date_inscription_detail_beneficiaire = date('Y-m-d', PHPExcel_Shared_Date::ExcelToPHP($date_inscription_detail_beneficiaire)); 
									}
								} else {
		...
		if($erreur_titre==0) { // c'est à dire le format est OK		
						...
						if( $menage_ou_individu=="menage" || $menage_ou_individu=="groupe") {
							if(strtolower($chef_menage) =="oui") {
								...
								if($nombre >0) {
									...
									$sheet->setCellValue("AE".$ligne, "Doublon : Déjà bénéficiaire de l'intervention ".$intitule_intervention_original);
									$nombre_erreur = $nombre_erreur + 1;	
									$erreur_doublon_menage= $erreur_doublon_menage + 1;	
								}						
							} else {
			...
			if($nombre_erreur > 0 || $erreur_titre >0) {
				// Signaler les erreurs par mail
				...
				$val_ret["date_inscription"] = $date_inscription_beneficiaire;
				$val_ret["nombre_erreur"] = $nombre_erreur;				
				$val_ret["erreur_titre"] = $erreur_titre;				
				$newchiffrelettre = new chiffreEnLettre;
				$nombre_erreur_en_lettre= $newchiffrelettre->ConvNumberLetter($nombre_erreur,0,0);
				...
				if($nombre_erreur > 0) {
					...
					if($erreur_doublon_menage>0) {					
						$objPHPExcel->setCellValue("A".$ligne, "Ménage doublon");					
						$objPHPExcel->setCellValue("B".$ligne, $erreur_doublon_menage);					
						$objPHPExcel->getStyle("A".$ligne.":B".$ligne)->applyFromArray($styleArray);	
						$objPHPExcel->getStyle("A".$ligne)->getAlignment()->setWrapText(true);		
						$ligne=$ligne + 1;						
					}	

...
class Financement_programme extends REST_Controller {
    ...
    public function index_post() {
		...
		$budget_initial=null;
		$temporaire=$this->post('budget_initial');
		if(isset($temporaire) && $temporaire !="" && $temporaire !="null" && intval($temporaire) >0) {
			$budget_initial=$temporaire;
		}
		$budget_modifie=null;
		$temporaire=$this->post('budget_modifie');
		if(isset($temporaire) && $temporaire !="" && $temporaire !="null" && intval($temporaire) >0) {
			$budget_modifie=$temporaire;
		}
